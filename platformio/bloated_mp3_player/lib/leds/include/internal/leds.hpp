...
namespace My
{
    namespace LED
    {
         ...
         */
        static inline Colour get_colour_from_pointer(const Colour *ptr)
        {
            Colour color;
         ...
         */
        static const Colour *const color_list[] PROGMEM = {
        ...
        constexpr size_t color_list_size = sizeof(color_list) / sizeof(color_list[0]);
         ...
         */
        static inline Colour read_colour_from_list(int16_t index = -1)
        {
            // Random selection
            if (index < 0) {
                index = random(color_list_size);
            }

            // Bounds check
            if ((size_t)index >= color_list_size) {
                index = 0;
            }

            // Read pointer from PROGMEM
            const Colour *ptr = (const Colour *)pgm_read_ptr(&color_list[index]);

            // Copy color from PROGMEM into RAM
            return get_colour_from_pointer(ptr);
        }
         ...
         */
        static inline size_t colourpos_length(const ColourPos *items)
        {
            size_t len = 0;
        ...
        class LED {
             ...
             */
            inline bool is_led_enabled() const
            {
                return _leds_enabled;
            }
             ...
             * @param count Number of LEDs to set (defaults to all).
             */
            void set_colour_from_list(const int16_t index = 0, const uint32_t duration = 0, const int16_t count = -1);
             ...
             */
            void set_led_position_from_list(const uint16_t led_index, const int16_t colour_index = 0, const uint32_t duration = 0, const bool refresh = true);
             ...
             * @param duration Duration in milliseconds to display (0 = infinite).
             */
            inline void set_colour_from_offset_from_list(const uint16_t start_index = 0, const uint16_t end_index = 25 - 1, const int16_t index_foreground = -1, const int16_t index_background = -1, const uint32_t duration = 0)
            {
                const Colour foreground = read_colour_from_list(index_foreground);
                const Colour background = read_colour_from_list(index_background);
                set_colour_from_offset(start_index, end_index, foreground, background, duration);
            }
            ...
            inline void display_percentage(const My::LED::Colour &fg, const My::LED::Colour &bg, const int16_t current, const int16_t max_steps)
            {
                int16_t progress = My::Utils::leds_for_progress<int16_t>(
                    current,
                    max_steps,
                    static_cast<int16_t>(_led_number)
                );
                set_colour(fg, LED_DURATION, progress, bg);
            };
             ...
             */
            inline void _update_forced_color_duration()
            {
                if (_forced_colour && _forced_colour_end_time > 0 && millis() >= _forced_colour_end_time) {
                    _forced_colour = false;            // allow automatic colors again
                    _forced_colour_end_time = 0;       // reset timer
                }
            }
             ...
             */
            inline int16_t _clamp_count(const int16_t count)
            {
                if (count < 0 || count > _led_number) {
                    return _led_number;
                }
                return count;
            }
             ...
             */
            inline int16_t _clamp_index_inclusif(const int16_t count)
            {
                if (count < 0 || count >= _led_number) {
                    return _led_number - 1;
                }
                return count;
            }
             ...
             */
            void _move_pixel(ColourPos &item, const uint16_t pos);
            const uint32_t _led_number;
            const uint8_t _led_pin;
            const uint8_t _led_brightness;
            const neoPixelType _led_type;
            /** External NeoPixel strip instance (defined in the companion source file). */
            Adafruit_NeoPixel _led_strip;
            /** True once `led_init()` has successfully initialised `LedStrip`. */
            bool _led_strip_initialized;
             ...
             */
            uint32_t _forced_colour_end_time;
