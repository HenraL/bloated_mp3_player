#ifndef BAKEDFONTS_WORKBENCH_8PT_H_
#define BAKEDFONTS_WORKBENCH_8PT_H_

#include <stdint.h>
#include <Arduino.h>
#include "font_structs.hpp"

namespace BakedFonts {

#define WORKBENCH_8PT_WIDTH  6
#define WORKBENCH_8PT_HEIGHT 8
#define WORKBENCH_8PT_FIRST  32
#define WORKBENCH_8PT_LAST   8725
#define WORKBENCH_8PT_COUNT  215

extern const uint16_t workbench_8pt_codes[];

extern const uint8_t workbench_8pt_bits[] PROGMEM;
extern const uint8_t workbench_8pt_widths[];

}  // namespace BakedFonts

#endif /* BAKEDFONTS_WORKBENCH_8PT_H_ */
