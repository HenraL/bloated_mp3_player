...
> The user frequently edits the config. **Always re-read these files before assuming pinouts or
> debug behaviour** — never trust earlier session memory or AGENTS.md snapshots over the current file.

Key config files (all under `platformio/bloated_mp3_player/include/my/config/`):

- `pins.hpp` — `My::Config::Pins`: LCD (SPI to GMG12864-06D/ST7565R: CS=10, RST=9, DC=8, SCLK=7, MOSI=6),
  IC font ROM (CS=5), I2C (SDA=2, SCL=1), SDMMC (CLK=39, CMD=38, D0=40), I2S audio
  (BCLK=16, WS=47, DIN=17, DIN2=18), **rotary pot (A=42, B=15, SW=12)**, ultrasonic (TRIG=14, ECHO=13),
  matrix LED (41), onboard LED (48), boot button (0).
- `debug.hpp` — `My::Config::Debug`: every `UART_*` bool gates a `serial_debug` line.
  `UART_PROFILING_ENABLED` (default `false`) turns the flamegraph `PROFILING:` stream on/off.
  `UART_STICK_RAW` / `UART_STICK_RAW_SW` dump raw A/B/SW pot levels on change (for diagnosing
  wiring vs firmware) — uses `Rotary::raw_changed()` in `lib/rotary/src/rotary.cpp`.

### Debugging workflow

Enable a `UART_*` flag in `debug.hpp`, rebuild, flash; serial output is buffered through
the single serial task (`my_tasks_serial.cpp`) — hot paths should use `serial_print_raw()`.
